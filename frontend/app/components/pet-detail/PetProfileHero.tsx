...
export default function PetProfileHero({ pet }: { pet: any }) {
  return (
    <section className="max-w-7xl mx-auto px-6 py-10 md:py-16">
      <div className="w-full max-w-[950px] mx-auto">
        <Link href="/find-pet" className="font-menu text-gray-500 font-bold text-[14px] flex items-center gap-1 mb-8 uppercase tracking-wider hover:text-[#f08c50] transition-colors w-max">
        ...
        <div className="flex flex-col md:flex-row gap-12 md:gap-16 lg:gap-20 items-center md:items-start justify-between pt-4">
          ...
          <div className="relative w-[280px] h-[280px] md:w-[350px] md:h-[350px]">
            ...
            <div className="absolute inset-0 z-20 overflow-hidden bg-white">
              <Image 
                src={pet?.primary_image || "https://images.unsplash.com/photo-1544568100-847a948585b9?q=80&w=600&auto=format&fit=crop"}
                alt={pet?.name || "Pet"} 
                fill 
          ...
          <div className="mt-6 md:mt-0 w-full md:w-[400px] lg:w-[450px] flex flex-col justify-start">
            ...
            <p className="font-menu text-[15px] text-gray-600 mb-8 border-b border-gray-100 pb-8 flex items-center gap-2">
              <span className="material-symbols-outlined text-[#f08c50] text-[18px]">location_on</span>
            ...
            <div className="bg-white border border-gray-200 rounded px-6 py-4 flex flex-col shadow-sm max-w-[320px]">
               <p className="font-menu text-gray-600 text-[15px] font-medium mb-4 flex items-center justify-between">
                 Adoption Fee: <strong className="text-[18px] text-gray-800 ml-2 font-black">${pet?.adoption_fee || '250'}</strong>
               </p>
               <Link href={`/adopt/${pet?.id || 1}/apply`} className="w-full">
